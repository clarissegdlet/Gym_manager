...
import static com.smartgym.manager.domain.BookingAsserts.*;
...
class BookingResourceIT {
    ...
    public static Booking createEntity(EntityManager em) {
        Booking booking = new Booking().bookingStatus(DEFAULT_BOOKING_STATUS).bookingTime(DEFAULT_BOOKING_TIME);
        // Add required entity
        Member member;
        ...
        if (TestUtil.findAll(em, ClassSession.class).isEmpty()) {
            classSession = ClassSessionResourceIT.createEntity();
            em.persist(classSession);
            em.flush();
        } else {
            classSession = TestUtil.findAll(em, ClassSession.class).get(0);
        }
    ...
    public static Booking createUpdatedEntity(EntityManager em) {
        Booking updatedBooking = new Booking().bookingStatus(UPDATED_BOOKING_STATUS).bookingTime(UPDATED_BOOKING_TIME);
        // Add required entity
        Member member;
        if (TestUtil.findAll(em, Member.class).isEmpty()) {
        ...
        updatedBooking.setMember(member);
        // Add required entity
        ClassSession classSession;
        if (TestUtil.findAll(em, ClassSession.class).isEmpty()) {
            classSession = ClassSessionResourceIT.createUpdatedEntity();
            em.persist(classSession);
            em.flush();
        } else {
            classSession = TestUtil.findAll(em, ClassSession.class).get(0);
        }
        updatedBooking.setClassSession(classSession);
    ...
    void putExistingBooking() throws Exception {
        ...
        updatedBooking.bookingStatus(UPDATED_BOOKING_STATUS).bookingTime(UPDATED_BOOKING_TIME);
