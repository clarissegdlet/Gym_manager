...
import static com.smartgym.manager.domain.PaymentAsserts.*;
...
import com.smartgym.manager.repository.PaymentRepository;
import com.smartgym.manager.service.dto.PaymentDTO;
import com.smartgym.manager.service.mapper.PaymentMapper;
...
class PaymentResourceIT {
    ...
    public static Payment createEntity(EntityManager em) {
        Payment payment = new Payment().amount(DEFAULT_AMOUNT).paymentDate(DEFAULT_PAYMENT_DATE).method(DEFAULT_METHOD);
        // Add required entity
        Membership membership;
        if (TestUtil.findAll(em, Membership.class).isEmpty()) {
            membership = MembershipResourceIT.createEntity(em);
            em.persist(membership);
            em.flush();
        } else {
            membership = TestUtil.findAll(em, Membership.class).get(0);
        }
    ...
    public static Payment createUpdatedEntity(EntityManager em) {
        Payment updatedPayment = new Payment().amount(UPDATED_AMOUNT).paymentDate(UPDATED_PAYMENT_DATE).method(UPDATED_METHOD);
        // Add required entity
        Membership membership;
        if (TestUtil.findAll(em, Membership.class).isEmpty()) {
            membership = MembershipResourceIT.createUpdatedEntity(em);
            em.persist(membership);
            em.flush();
        } else {
            membership = TestUtil.findAll(em, Membership.class).get(0);
        }
        updatedPayment.setMembership(membership);
